...
{
    /// <summary>
    /// A <see cref="PositiveValidator"/> instance that validates whether the value is more than 0.
    /// </summary>
    public class PositiveValidator : Validator
    {
        /// <summary>
        ...
        protected override void ValidateCore(object instance, string value, IList<ValidationResult> results)
        {
            ...
            if (double.TryParse(value, out numericValue))
            {
                if (numericValue <= 0)
                {
                    results.Add(new PropertyValidationResult(property, CachingResources.ValidationNumericValueIsOverZero, false));
                }
            }
        }
