...
namespace Microsoft.Practices.EnterpriseLibrary.Caching.Tests.IsolatedStorage
{
    ...
    public class FatFixture
    {
        ...
        [TestMethod]
        public void WhenQueriesForCompactSuggestionsWithSingleSequencies_ThenMovesFreeBlocksToEnd()
        {
            var stream = new MemoryStream();

            var fat = Fat.Initialize(stream, Offset, 32);

            var id1 = fat.AllocateBlockSequence(1).First();
            var id2 = fat.AllocateBlockSequence(1).First();
            fat.ReleaseBlockSequence(id1);

            var suggestion = fat.GetCompactSuggestions().Single();

            Assert.AreEqual(id2, suggestion.Item1);
            Assert.AreEqual(id1, suggestion.Item2);
            Assert.AreEqual(true, suggestion.Item3);

            Assert.AreEqual(id2, fat.GetCurrentBlockSequenceIds().Single());
        }

        [TestMethod]
        public void WhenQueriesForCompactSuggestionsWithNonHead_ThenMovesFreeBlocksToEnd()
        {
            var stream = new MemoryStream();
            ...
            var id1 = fat.AllocateBlockSequence(1).First();
            var sequence2 = fat.AllocateBlockSequence(2);
            fat.ReleaseBlockSequence(id1);

            var suggestion = fat.GetCompactSuggestions().Single();

            Assert.AreEqual(sequence2.ElementAt(1), suggestion.Item1);
            Assert.AreEqual(id1, suggestion.Item2);
            Assert.AreEqual(false, suggestion.Item3);
        }

        [TestMethod]
        public void WhenQueriesForCompactSuggestionsWithMultipleSequencies_ThenMovesFreeBlocksToEnd()
        {
            var stream = new MemoryStream();

            var fat = Fat.Initialize(stream, Offset, 32);

            var sequence1 = fat.AllocateBlockSequence(2);
            var sequence2 = fat.AllocateBlockSequence(2);
            var sequence3 = fat.AllocateBlockSequence(2);
            var sequence4 = fat.AllocateBlockSequence(2);
            var sequence5 = fat.AllocateBlockSequence(2);
            fat.ReleaseBlockSequence(sequence2.First());
            fat.ReleaseBlockSequence(sequence3.First());

            var suggestions = fat.GetCompactSuggestions();

            Assert.AreEqual(4, suggestions.Count());
            Assert.IsTrue(suggestions.Any(x => x.Item1 == sequence4.ElementAt(0) && x.Item3 == true));
            Assert.IsTrue(suggestions.Any(x => x.Item1 == sequence4.ElementAt(1) && x.Item3 == false));

            Assert.IsTrue(suggestions.Any(x => x.Item1 == sequence5.ElementAt(0) && x.Item3 == true));
            Assert.IsTrue(suggestions.Any(x => x.Item1 == sequence5.ElementAt(1) && x.Item3 == false));

            Assert.IsTrue(suggestions.Any(x => x.Item2 == sequence2.ElementAt(0)));
            Assert.IsTrue(suggestions.Any(x => x.Item2 == sequence2.ElementAt(1)));
        ...
        [TestMethod]
        public void WhenQueriesForCompactSuggestionsWithNoRemovals_ThenReturnsEmpty()
        {
            var stream = new MemoryStream();
            ...
            Assert.AreEqual(0, fat.GetCompactSuggestions().Count());
        }

        [TestMethod]
        public void WhenAcceptingCompactSuggestion_ThenUpdatesFat()
        {
            var stream = new MemoryStream();
            ...
            fat.AcceptCompactSuggestion(suggestion);

            Assert.AreEqual(id1, fat.GetCurrentBlockSequenceIds().Single());
        }

        [TestMethod]
        public void WhenAcceptingCompactSuggestionWithNonHead_ThenUpdatesFat()
        {
            var stream = new MemoryStream();
            ...
            var suggestion = fat.GetCompactSuggestions().Single();
            Assert.AreEqual(sequence2.First(), fat.GetCurrentBlockSequenceIds().Single());

            fat.AcceptCompactSuggestion(suggestion);

            Assert.AreEqual(sequence2.First(), fat.GetCurrentBlockSequenceIds().Single());
            var sequence2updated = fat.GetBlocksInSequence(sequence2.First());

            Assert.AreEqual(id1, sequence2updated.ElementAt(1));
        }

        [TestMethod]
        public void WhenTrimmingWithEmptyBlocksInTheEnd_ThenReducesSize()
        {
            var stream = new MemoryStream();

            var fat = Fat.Initialize(stream, Offset, 32);

            var id1 = fat.AllocateBlockSequence(2).First();
            var size = stream.Length;

            var id2 = fat.AllocateBlockSequence(2).First();
            Assert.AreEqual(size + sizeof(int)*2, stream.Length);
            fat.ReleaseBlockSequence(id2);
            Assert.AreEqual(size + sizeof(int)*2, stream.Length);
        ...
        [TestMethod]
        public void WhenTrimmingWithEmptyBlocksInTheMiddle_ThenDoesNotReduceSize()
        {
            var stream = new MemoryStream();
