...
namespace Microsoft.Practices.EnterpriseLibrary.Logging.Tests.TraceListeners.IsolatedStorage.give_storage_with_entries
{
    [TestClass]
    [Tag("IsolatedStorage")]
    public class when_adding_entry_larger_than_remaining_space : Context
    {
        private byte[] entry1;
        ...
        protected override void Act()
        {
            ...
            this.entry1 = Enumerable.Range(0, 384).Select(i => (byte)i).ToArray();

            this.storage.Add(this.entry1);
        }

        [TestMethod]
        public void then_header_points_to_second_entry()
        {
            Assert.AreEqual(
        ...
        public void then_can_read_entries_from_storage()
        {
            ...
            CollectionAssert.AreEqual(
                new[] { this.existingEntry2, this.existingEntry3, this.entry1 },
                entries.ToArray(),
                new EntryComparer());
        }
    }
